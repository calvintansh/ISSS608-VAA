...
Identifying columns for Sailor Shift, Ivy Echos and Oceanus Folk are added to aid identification and filtering for further analysis.
...
For Edges, there are duplicates but only their *key* are different. Since, information on *key* were not provided by VAST 2024 MC1, their purpose cannot be determined. Therefore, duplicated edges differing only by their *key* will be removed.
...
However, this is not enough because in Section 2.4, duplicated Nodes and their respective *id* (now called *name*) were removed. Therefore, without treating edges, there will be edges that do not have a match to nodes since they are matching the removed *id*.
...
## [2:]{style="color:DarkOrange"} 3.8 Creating the Global Knowledge Graph
...
Since several of the ggraph layouts involve randomisation, the code below is used to set the seed value to ensure reproducibility of plots.
...
First, Sailor Shift's node is extracted, and all outgoing edges from her are identified. Then, the connected nodes are filtered to include only Songs and Albums.
...
The table below shows all Songs and Albums that Sailor Shift has produced in her music career.
...
### [2:]{style="color:DarkOrange"} 4.2.1 Data Preparation Sailor Shift
...
# Step 4: Define relevant edge types
creator_edge_types <- c("PerformerOf", "ComposerOf", "ProducerOf", "LyricistOf")

# Step 5: Identify source nodes of creator edges pointing to those neighbours
creator_edges <- mc1_edges_clean %>%
  filter(to %in% sailor_music_out_nodes, `Edge Type` %in% creator_edge_types)
...
# Step 7: Filter graph to relevant nodes only
sub_graph <- graph %>%
  filter(name %in% sailor_all_node_names)
```

### [2:]{style="color:DarkOrange"} 4.2.2 Visualisation Sailor Shift
...
::: callout-tip
## Sailor Shift's Influence Insights

-   Sailor Shift is a member of Ivy Echos
-   Therefore, those who influence Ivy Echos will also influence her
-   Need to create Ivy Echos influence graph
:::

### [2:]{style="color:DarkOrange"} 4.2.3 Data Preparation Ivy Echos
...
creator_edges <- mc1_edges_clean %>%
  filter(to %in% ivy_music_out_nodes, `Edge Type` %in% creator_edge_types)
...
### [2:]{style="color:DarkOrange"} 4.2.4 Visualisation Ivy Echos
...
g <- sub_graph %>%
  ...
  geom_node_text(
    aes(
      label = ifelse(`node_name` == "Ivy Echos", "Ivy Echos", NA)
    ),
    size = 4,
    colour = 'red',
    show.legend = FALSE
...
### [2:]{style="color:DarkOrange"} 4.2.5 Data Preparation for Combined Graph

The Sailor Shift and Ivy Echos visualisation will be combined now to produce a holistic graph of who influenced Sailor Shift.
...
### [2:]{style="color:DarkOrange"} 4.2.6 Visualisation for Combined Graph
...
g <- sub_graph %>%
  ...
  geom_edge_fan(
    ...
    tooltip = function(data) data$`Edge Type`,
    arrow = arrow(length = unit(2, 'mm')),
    alpha = 0.4
    ) +
  geom_point_interactive(
  ...
  geom_node_text(
    aes(
      label = ifelse(`node_name` == "Sailor Shift", "Sailor Shift", ifelse(`node_name` == "Ivy Echos", "Ivy Echos", ifelse(`node_name` == "Wei Zhao", "Wei Zhao", NA)))
    ),
    size = 4,
    colour = 'red',
    show.legend = FALSE
...
### [2:]{style="color:DarkOrange"} 4.3.1 Data Preparation Sailor Shift

The start uses sailor_music_all from Section 4.2.1 and filters out the music with more that 2 inward edges. This removes music that she produced that has no collaborators or influences. However, 2 of her music (The Current & The Chord and Salty Dreams) has 2 inward edges and have to be manually removed. Then all in neighbours to her music are found. These contains these are the Songs and Albums that her music have influenced as well as Person or Musical Groups that she has collaborated with. A filter is applied to retain Songs and Albums and the inward neighbours to those are found. These are the Person and Musical Groups that produced the music that her music have been influenced by as well as Songs and Albums that have been influenced by the music that her music have been influenced by.
...
### [2:]{style="color:DarkOrange"} 4.3.2 Visualisation Sailor Shift
...
::: callout-tip
## Sailor Shift's Insights

-   The graph shows that Sailor Shift collaborated with many Persons and Musical Groups
-   However, it seems that no music have been influenced by the music that she produced
-   The influence of Ivy Echos will be investigated in the next section
:::

### [2:]{style="color:DarkOrange"} 4.3.3 Data Preparation Ivy Echos
...
### [2:]{style="color:DarkOrange"} 4.3.4 Visualisation Ivy Echos
...
g <- sub_graph %>%
  ...
  geom_node_text(
    aes(
      label = ifelse(`node_name` == "Sailor Shift", "Sailor Shift", ifelse(`node_name` == "Ivy Echos", "Ivy Echos",NA))
    ),
    size = 4,
    colour = 'red',
    show.legend = FALSE
...
### [2:]{style="color:DarkOrange"} 4.3.3 Data Preparation Ivy Echos Music
...
### [2:]{style="color:DarkOrange"} 4.3.4 Visualisation Ivy Echos Music
...
### [2:]{style="color:DarkOrange"} 4.3.5 Data Preparation for Combined Graph
...
### [2:]{style="color:DarkOrange"} 4.3.6 Visualisation for Combined Graph
...
The tables below shows all the Persons and Musical Groups that she has collaborated with directly and directly or indirectly influenced - The first graph and table shows everyone that she has collaborated with excluding Ivy Echos - The graph of Sailor Shift shows that she has collaborated with numerous different artists and information in them can be found in the table. - However, none of her music has inspired others, as no Songs or Albums reference her Songs or Albums. - The second graph and table shows all members of Ivy Echos - Being part of the same group, Sailor Shift has collaborated with them to produce a few Songs / Albums. - The third graph and table shows the influence of Ivy Echos - Ivy Echos has collaborated with a few Persons and Group - Unlike Sailor Shift, one of Ivy Echo's songs has influenced an Album. The final table displays the affected Person and Musical Group. - Neither Sailor Shift nor Ivy Echos has indirectly influenced anyone, as no Songs or Albums reference their works in a secondary chain (i.e., no Songs/Albums refer to other Songs/Albums that were themselves influenced by theirs).
:::

#### Sailor Shift's Direct Collaborators excluding Ivy Echos
...
#### Sailor Shift's Team Mates in Ivy Echos
...
#### Ivy Echos' Direct Collaborators
...
graph_no_member = tbl_graph(edges = mc1_edges_no_member,
                             nodes = mc1_nodes_clean,
                             directed = TRUE)

# Convert tbl_graph to igraph
ig <- as.igraph(graph_no_member)
```
...
sub_graph_no_member <- graph_no_member %>%
