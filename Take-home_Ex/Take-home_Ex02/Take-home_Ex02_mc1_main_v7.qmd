...
-  Qian Yuan is the most influential artist having influenced 92 music, 388 artists and being referenced to 489 times.
    - Jay Walters has a much lower influence having influenced 10 music, 18 artists and being referenced to 87 times.
...
This will be a continuation of Question 3a where only Oceanus Folk artists are analysed. In order to predict the next Oceanus Folk stars, a grading rubrix will be used where their ranking in these 4 categories (Number of Music released, Number of Notable Music, Number of Music influenced, Number of New Artists Influenced) will provide their overall score.
...
oceanus_creator_and_songs_and_influences_and_creators <- creator_and_songs_and_influences_and_creators %>%
  filter(creator_from %in% in_neighbours)
...
oceanus_creator_stats %>%
  arrange(desc(total_songs)) %>%
  head(10) %>%
  kable(caption = "Top 10 Oceanus Folk Artists by Number of Music Produced") %>%
  kable_styling("striped", full_width = F) %>% 
  scroll_box(height = "200px")
...
oceanus_creator_stats %>%
  arrange(desc(notable_hits)) %>%
  head(10) %>%
  kable(caption = "Top 10 Oceanus Folk Artists by Number of Notable Hits") %>%
  kable_styling("striped", full_width = F) %>% 
  scroll_box(height = "200px")
...
oceanus_creator_stats %>%
  arrange(desc(influence_music)) %>%
  head(10) %>%
  kable(caption = "Top 10 Oceanus Folk Artists by Musical Influence") %>%
  kable_styling("striped", full_width = F) %>% 
  scroll_box(height = "200px")
...
oceanus_creator_stats %>%
  arrange(desc(influence_music)) %>%
  head(10) %>%
  kable(caption = "Top 10 Oceanus Folk Artists by Number of Influencd Artists") %>%
  kable_styling("striped", full_width = F) %>% 
  scroll_box(height = "200px")
...
oceanus_creator_stats %>%
  arrange(desc(reference_freq)) %>%
  head(10) %>%
  kable(caption = "Top 10 Oceanus Folk Artists by Frequency of References from Artists") %>%
  kable_styling("striped", full_width = F) %>% 
  scroll_box(height = "200px")
```