...
The code groups different types of edges and nodes into broader categories for consistent colouring in the graphs.

```{r}
mc1_edges_clean <- mc1_edges_clean %>%
  mutate(`Edge Colour` = case_when(
    `Edge Type` %in% c("PerformerOf", "ComposerOf", "ProducerOf", "LyricistOf", "RecordedBy", "DistributedBy") ~ "Creater Of",
...
girafe(ggobj = g, width_svg = 8, height_svg = 7)
```
...
- It seems the Oceanus Folk community places a strong emphasis on collaboration
- Rather than simply drawing influence from each other’s work, they prefer to create music together directly
:::