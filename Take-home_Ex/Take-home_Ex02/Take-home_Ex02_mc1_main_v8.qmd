...
# Step 2: Get all outgoing edges from these music
music_out_edges <- mc1_edges_clean %>%
  filter(from %in% all_music, `Edge Colour` == "Influenced By")

# Step 3: Get all songs that has influenced the original song
influenced_music <- mc1_nodes_clean %>%
  filter(name %in% music_out_edges$to, genre == "Oceanus Folk") %>%
  pull(name)

all_nodes <- unique(c(
                      influenced_music))

sort(influenced_music)
```
...
-   Their last Oceaus Folk work was in 2017 and was last refereed to in 2030.
