...
# Step 7: For each release date, count distinct creators
...
# First join to get music names (from)
music1 <- all_music %>%
...
# Second join to get influenced music names (to)
music2 <- music1 %>%
...
genre_influence_stats <- music2 %>%
  ...
  summarize(
    ...
    other_influences = total_influences - oceanus_influences - no_influences,
...
-   The Oceanus Folk genre has been the most influenced by itself with about a third of all music (Songs/Albums) drawing inspiration from Oceanus Folk music.
...
# Step 3: Get all songs made by creators that are not oceanus folk
creator_music <- mc1_nodes_clean %>%
  filter(name %in% creator_out_edges$to, genre != "Oceanus Folk")
...
# Third join to get song's influenced genre (to)
creator_and_songs_and_influences <- creator_and_songs %>%
  ...
  select(from, creator_name, creator_node_type, song_to, song_name, release_date, song_genre, notable, influenced_to, influenced_genre) %>%
  distinct()

# To find songs influenced by Oceanus Folk
creator_and_songs_and_influences_compressed <- creator_and_songs_and_influences %>%
  group_by(creator_name, creator_node_type,song_name, release_date, song_genre, notable) %>%
  summarize(
    influenced_by_oceanus = ifelse(
      any(influenced_genre == "Oceanus Folk", na.rm = TRUE),
      TRUE,
      FALSE
    ),
    .groups = "drop"
...
creator_stats <- creator_and_songs_and_influences_compressed %>%
  group_by(creator_name, creator_node_type) %>%
  summarize(
    total_songs = n_distinct(song_name),
    oceanus_folk_songs = sum(song_genre == "Oceanus Folk", na.rm = TRUE),
    oceanus_influence = sum(influenced_by_oceanus == TRUE, na.rm = TRUE),
    notable_hits = sum(notable == TRUE, na.rm = TRUE),
    perc_oceanus = round((oceanus_folk_songs + oceanus_influence) / total_songs * 100, 1)
  ) %>%
...
top_creators <- creator_and_songs_and_influences %>%
  filter(creator_name %in% creator_stats$creator_name) %>%
  pull(from)

top_creators_songs <- creator_and_songs_and_influences %>%
  filter(creator_name %in% creator_stats$creator_name) %>%
  pull(song_to)

top_creators_songs_influenced <- creator_and_songs_and_influences %>%
  filter(creator_name %in% creator_stats$creator_name, 
         !is.na(influenced_to), 
...
sub_graph <- sub_graph %>%
  left_join(creator_stats %>% select(creator_name, perc_oceanus),
...
g <- sub_graph %>%
  ...
  geom_point_interactive(
    aes(
      ...
      colour = `Node Colour`,
      shape = `Node Type`,
      size = ifelse(name %in% top_creators, perc_oceanus, 1),
      tooltip = case_when(
        `Node Type` == "Album" ~ sprintf(
          "%s<br/>%s<br/>Notable: %s<br/>(%s)", node_name, genre, notable, release_date
        ),
        ...
        TRUE ~ sprintf("%s<br/>perc_oceanus: %s", node_name, perc_oceanus)
      )
    )
  )+ 
  geom_node_text(
...
-   The table above show the top 10 artists who was most influenced by Oceanus Folk
    ...
    - These artists are rank by the number of notable hits that they have.
- The top artist is Jing Kang who has produced one Oceanus Folk song and has an influence score of 3.4%.
- Next is Sailor Shift who has produced 36 Oceanus Folk songs with an influence score of 94.7%.
:::
