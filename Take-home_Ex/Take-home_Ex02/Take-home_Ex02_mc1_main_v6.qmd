...
# Step 9: Fourth join to itself influenced song's creator
...
creator_stats <- creator_and_songs_and_influences_and_creators_compressed %>%
  ...
  summarize(
    ...
    total_oceanus_influence = oceanus_songs + oceanus_influence,
...
# Step 2: Get the songs that the top creators produced
...
sub_graph <- sub_graph %>%
  left_join(creator_stats %>% select(creator_name, total_oceanus_influence),
...
g <- sub_graph %>%
  ...
  geom_point_interactive(
    aes(
      ...
      colour = `Node Colour`,
      shape = `Node Type`,
      size = ifelse(name %in% top_creators, total_oceanus_influence, 1),
      tooltip = case_when(
        `Node Type` == "Album" ~ sprintf(
          "%s<br/>%s<br/>Notable: %s<br/>(%s)", node_name, genre, notable, release_date
        ),
        ...
        TRUE ~ sprintf("%s<br/>perc_oceanus: %s", node_name, total_oceanus_influence)
      )
    )
  )+ 
  geom_node_text(
...
## [2:]{style="color:DarkOrange"} 6.1 Question 3a) Visualize the careers of three artists. Compare and contrast their rise in popularity and influence?
...
creator_and_songs <- creators %>%
  ...
  rename(creator_from = from, song_name = node_name, song_genre = genre, song_to = to) %>%
  distinct()

# Third join to get influenced songs (to)
creator_and_songs_and_influences <- creator_and_songs %>%
  left_join(creator_songs_out_edges %>% select(from, to), by = c("song_to" = "to")) %>%
  left_join(mc1_nodes_clean %>% select(name, genre, node_name), by = c("from" = "name")) %>%
  rename(influence_genre = genre, influence_from = from, infuence_name = node_name) %>%
  select(creator_from, creator_name, creator_node_type, song_to, song_name, release_date, song_genre, notable, influence_from, infuence_name, influence_genre) %>%
  distinct()

# First get song-level stats
song_stats <- creator_and_songs_and_influences %>%
  group_by(creator_name, creator_node_type, song_to, song_name, song_genre, notable) %>%
  summarize(influence_count = n_distinct(influence_from),
            influence_genre_count = n_distinct(influence_genre))

# Then aggregate to creator level
creator_stats <- song_stats %>%
  group_by(creator_name, creator_node_type) %>%
  summarize(
    total_songs = n_distinct(song_to),
    total_genre = n_distinct(song_genre),
    notable_hits = sum(notable == TRUE, na.rm = TRUE),
    total_influence = sum(influence_count),
    total_influence_genre = sum(influence_genre_count)
  ) %>%
...
# Step 1: Get all creator names
chosen_creator <- mc1_nodes_clean %>%
  filter(`Node Type` %in% c("Person", "MusicalGroup"), node_name == "Guiying Liao") %>%
  pull(name)

# Step 2: Get all outgoing edges from these creators
creator_out_edges <- mc1_edges_clean %>%
  filter(from %in% chosen_creator, `Edge Colour` == "Creator Of")
...
# Step 4: Get all incoming edges from songs
creator_songs_in_edges <- mc1_edges_clean %>%
  filter(to %in% creator_music, `Edge Colour` == "Influenced By")

# Step 3: Get all songs influenced by creator's songs
influenced_music <- mc1_nodes_clean %>%
  filter(name %in% creator_songs_in_edges$from) %>%
  pull(name)

# Step 4: Get all incoming edges from these influenced songs
influenced_music_in_edges <- mc1_edges_clean %>%
  filter(to %in% influenced_music, `Edge Colour` == "Creator Of")

# Step 3: Get all influenced creators
influenced_music_creators <- mc1_nodes_clean %>%
  filter(name %in% influenced_music_in_edges$from) %>%
  pull(name)
```
...
# Combine relevant nodes
all_nodes <- unique(c(chosen_creator,
                      creator_music))
...
```{r}
mc1_nodes_clean %>%
  filter(name %in% creator_music)
```

## [2:]{style="color:DarkOrange"} 6.2 Question 3b) Using this characterization, give three predictions of who the next Oceanus Folk stars with be over the next five years.
