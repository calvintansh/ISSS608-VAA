...
## [2:]{style="color:DarkOrange"} 3.3 Mapping from Node name to Edges Indexes

igraph uses the *from* and *to* columns in the edges and matches them with the *name* column in nodes. The code below renames *name* column to *person_name* and then renames *id* column to *name* to match the *from* and *to* columns in the edges

```{r}
mc1_nodes_clean <- mc1_nodes_clean %>%
  rename(person_name = name, name = id) %>%
...
## [2:]{style="color:DarkOrange"} 3.4 Map Source and Target to Node Name
...
Sailor Shift is a unique value in *person_name* so it can be used for identification instead of the *name* column.

```{r}
# Find Sailor Shift by name
mc1_nodes_clean %>%
  filter(person_name == "Sailor Shift")
```
...
g <- sub_music %>%
  ...
  geom_point_interactive(
    aes(
      ...
      y = y,
      data_id = name,
      tooltip = sprintf("%s<br/>(%s)<br/>(%s)<br/>(%s)", person_name, release_date, notable, name),
      colour = genre,
      shape = `Node Type`
      ),
  ...
  geom_node_text(
    aes(
      label = ifelse(`person_name` == "Sailor Shift", "Sailor Shift", NA)
    ),
    size = 4,
    colour = 'red',
    show.legend = FALSE
...
mc1_nodes_clean %>%
  ...
  select(`Node Type`, person_name, release_date, genre, notable, single, notoriety_date)
```
...
g <- sub_music %>%
  ...
  geom_point_interactive(
    aes(
      ...
      y = y,
      data_id = name,
      tooltip = sprintf("%s<br/>(%s)", person_name, release_date),
      colour = genre,
      shape = `Node Type`
      ),
    show.legend = c(size = FALSE)
...
Ivy Echos is a unique value in *person_name* so it can be used for identification instead of the *name* column.

```{r}
mc1_nodes_clean %>%
  filter(person_name == "Ivy Echos")
```
...
excluded_ids <- mc1_nodes_clean %>%
  filter(person_name %in% excluded_names) %>%
  pull(name)
...
# Step 6: Create tbl_graph subgraph using only those nodes and relevant edges
sub_music <- music %>%
  filter(name %in% sailor_all_node_names)
```
...
ivy_all_node_names <- unique(c(
  ...
  in_edges_to_out_neighbours_songs_neighbours
))
