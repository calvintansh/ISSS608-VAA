...
- The legend was removed and background was removed using *theme()* and the axis titles were removed using *labs()* for a cleaner plot.

Zoom Rectangle: 

- Created as a transparent dotted line rectangle to highlight the zoomed-in location using *cowplot()*.

Overlaid Plots: 

- *ggdraw()* was used to create an empty canvas, and *draw_plot()* was used to overlay s1, s2, and the zoom rectangles. These functions are part of the *cowplot* package and allows the zoom-in area to be highlighted.
...
- *labs()* was used to remove the x-axis and label the y-axis.
